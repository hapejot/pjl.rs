...
mod testing {
    ...
    use crate::{data::model::DataModel, error::Error, ser::CopyRule, SqlValue};
    ...
    fn row_serializer() -> Result<(), Error> {
        ...
        let rs = crate::ser::serialize_row(Rc::new(model), p)?;
    ...
    fn order_serializer() {
        ...
        let rs = crate::ser::serialize_row(Rc::new(model), o).unwrap();
        assert_eq!(4, rs.len());
        for r in rs.iter() {
            match r.table() {
                "Order" => {

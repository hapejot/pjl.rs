...
async fn api_list_records(Path(entity): Path<String>, state: Arc<AppState>) -> impl IntoResponse {
                ...
                if let Ok(content) = fs::read_to_string(&path) {
                    if let Ok(record) = serde_yaml::from_str::<serde_yaml::Value>(&content) {
                        ...
                        if let Some(obj) = json_record.as_object_mut() {
                            obj.insert("__metadata".to_string(), json!({
    ...
    odata_json_response(StatusCode::OK, json!({"d": {"results":records}}))
}
...
async fn api_get_record(
    ...
    if record.is_null() {
        ...
        odata_json_response(StatusCode::OK, record)
    }
}
...
async fn api_save_record(
    ...
    let yaml = match serde_yaml::to_string(&record) {
        ...
        Err(e) => {
            return odata_json_response(
                ...
                json!({"error": format!("YAML serialization error: {}", e)}),
            );
        }
    };
    let data_dir = format!("data/{}", entity);
    if let Err(e) = create_dir_all(&data_dir) {
        return odata_json_response(
            ...
            json!({"error": format!("Failed to create data dir: {}", e)}),
        );
    }
    let file_path = format!("{}/{}-{}.yaml", data_dir, entity, id);
    ...
    odata_json_response(StatusCode::OK, json!({"id": id}))
}
...
async fn odata_metadata(state: Arc<AppState>) -> impl IntoResponse {
    ...
    // Example: fixed last-modified date stored in the app structure
    let last_modified = "Fri, 13 Jun 2025 10:00:00 GMT";
